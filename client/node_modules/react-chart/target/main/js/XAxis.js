...
module.exports = React.createClass({displayName: 'exports',
  render: function() {
    ...
    calcTicks(this.props.max, this.props.min).map(function(t) {
      ...
      var value = format(t);
      if (isX) {
        lines.push(React.DOM.line( {y2:-chartHeight, x1:p.x, x2:p.x, key:value}));
      } else {
        lines.push(React.DOM.line( {x2:chartWidth, y1:p.y, y2:p.y, key:value}));
      }
      labels.push(React.DOM.text( {x:textX || p.x, dy:dy, y:textY || p.y, key:value}, value));
    }, this);

    return React.DOM.g( {className:"chart-axis x", transform:"translate(0,420)"}, 
      React.DOM.g( {className:"lines"}, lines),
      React.DOM.line( {className:"axis", x2:chartWidth}),
      labels,
      React.DOM.text( {'text-anchor':"middle", y:"36", x:chartWidth/2}, "X Value"),
      React.DOM.text( {dy:dy, y:textY}, format(this.props.min)),
      React.DOM.text( {x:textX || chartWidth, dy:dy, y:textY || 0}, format(this.props.max))
    );
}});